...
def make_up_post_motion(pre_field: ti.template(), post_field: ti.template(),
    ...
    """make_up_post_motion 生成后半部分混合动画帧信息
    ...
    pre_frame = pre_field[0]#s-1
    begin_frame = pre_field[1] #s
    end_frame = post_field[0] #t
    post_frame = post_field[1] #t+1
    s1 = get_translation(begin_frame) - get_translation(pre_frame)
    s2 = get_translation(post_frame) - get_translation(end_frame)
    Pt = get_translation(begin_frame) + blend_frame_cnt * (abs(s1) + abs(s2)) * ((s1) / abs(s1)) * 1/2
    # 在这里添加你的实现
    # 将计算得到的根节点位置和旋转填写到下边的root_pos和root_rot中
    
    root_pos[None] = Pt
    root_rot[None] =  get_rot_matrix(begin_frame) @ tm.inverse(get_rot_matrix(end_frame))
...
def modify_root_transform(curr_transform: mat4, endframe_transform: mat4,
    ...
    """
    # 在这里添加你的实现
    dq = get_translation(curr_transform) - get_translation(endframe_transform)
    pos = root_rotation @ dq + root_pos
    rot = root_rotation
...
def blend_anim_slerp(start_frame: ti.template(), end_frame: ti.template(), t: float, result: ti.template()):
    ...
    for s in result:
        # 在这里添加你的实现
        begin_pos = get_translation(start_frame[s])
        end_pos = get_translation(end_frame[s])
        begin_rot = get_rot_matrix(start_frame[s])
        end_rot = get_rot_matrix(end_frame[s])
        begin_quaternion = matrix_to_quaternion(begin_rot)
        end_quaternion = matrix_to_quaternion(end_rot)
        ans_rot = slerp(begin_quaternion,end_quaternion,t)
        ans_pos = tm.mix(begin_pos,end_pos,t)
        # result[s] 表示新的关节s的变换矩阵
        result[s] = construct_transform_matrix(quaternion_to_matrix(ans_rot),ans_pos)
