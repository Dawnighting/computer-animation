...
class Spark:
    ...
    def ComputeForce(self,i):
        ...
        """
        self.state[i][4] = - KDrag * self.state[i][2]
        self.state[i][5] = -utils.GRAVITY * self.state[i][6] - KDrag * self.state[i][3]
        pass
    ...
    def FindDeriv(self,i):
        ...
        """
        # 你需要实现此函数的内容
        self.deriv[i][0] = self.state[i][2] * self.deltaT
        self.deriv[i][1] = self.state[i][3] * self.deltaT
        self.deriv[i][2] = self.deltaT * self.state[i][4] / self.state[i][6]
        self.deriv[i][3] = self.deltaT * self.state[i][5] / self.state[i][6]
        self.deriv[i][4] = 0
        self.deriv[i][5] = 0
        self.deriv[i][6] = 0
        self.deriv[i][7] = 0
    ...
    def UpdateState(self,i):
        ...
        if self.state[i][7] <= 0.0:
            self.mode[i] = 1
            
        else:
            if self.state[i][1] <= 0.0 and self.state[i][3] < 0.0:
                self.state[i][3] =  -1 * COR * self.state[i][3]
